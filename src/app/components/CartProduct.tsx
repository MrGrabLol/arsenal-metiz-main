...
export default function CartProduct() {
  return (
    <Link
      href="/marketplace/1"
      className="flex flex-col sm:flex-row sm:items-stretch justify-between gap-8 relative bg-white border border-brand-dark-gray-100/30 rounded-md smooth-200 p-5 sm:p-7.5"
    >
      {/* Remove Btn */}
      <button className="size-7 sm:size-10 shrink-0 absolute -top-3.5 -right-3.5 sm:-top-5 sm:-right-5 flex-center bg-brand-whitish-200 border border-brand-dark-gray/30 rounded-full cursor-pointer hover:bg-white smooth-200">
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"
      ...
      <div className="flex flex-col justify-between space-y-5">
        {/* Name */}
        <h4 className="font-medium text-xl/6.5 lg:text-[26px]/6.5">
          Проволока СВ08Г2С-О, ГОСТ 123-45
        </h4>
        {/* Details */}
        <ul className="space-y-2.5 sm:space-y-3.5 lg:text-[22px]/5.5">
      ...
      <div className="flex flex-col items-end sm:space-y-12">
        {/* Product Image */}
        <div className="size-20 sm:size-25 shrink-0 absolute sm:static bottom-5 left-5 sm:left-auto sm:top-7.5 sm:right-7.5 flex-center bg-brand-whitish-200 rounded-md">
          <Image
        ...
        {/* Cost */}
        <p className="whitespace-nowrap text-xl sm:text-[22px]/5.5 font-medium text-end pt-16 sm:pt-0">
          От 100000 ₽/Т
        </p>
      </div>
    </Link>
