...
export default function RegisterModal({ onToggle }: { onToggle: () => void }) {
  const registerRef = useRef(null);

  useClickOutside(registerRef, onToggle);
  ...
  return (
    ...
    >
      <div className="relative" ref={registerRef}>
        ...
        <div className="max-w-fit space-y-7.5 bg-white rounded-md px-6 py-9 sm:p-12.5">
          <h2 className="text-center">Регистрация</h2>

          <form className="max-w-115 w-full space-y-5">
            {/* Full Name */}
            ...
            <label htmlFor="password" className="block">
              <span className="md:text-xl/5 font-medium">Пароль</span>
              <input
                id="password"
                type="password"
                className="h-12 md:h-13.5 w-full leading-4 md:text-lg/4.5 bg-brand-whitish-200 placeholder:text-brand-dark/50 rounded-md outline-none border border-transparent focus:border-brand-dark-gray/30 mt-2.5 px-4 md:px-5"
